...
def eval_model(args):
    ...
    for i in tqdm(range(0,len(questions),args.batch_size)):
        ...
        for j in range(i,batch_end):
            ...
            input_ids = preprocess_qwen([{'from': 'human','value': qs},{'from': 'gpt','value': None}], tokenizer, has_image=True).cuda()
            ...
            image_folder.extend(image_tensor)
        ...
        image_tensor_batch = image_folder

        # image_tensor_batch = image_processor.preprocess(image_folder, return_tensors='pt')['pixel_values']
