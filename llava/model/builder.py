...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                    model = LlavaQwenForCausalLM.from_pretrained(model_base, ignore_mismatched_sizes=True, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
            ...
            elif "aerialchat" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                from llava.model.language_model.aerialchat_llava_qwen import AerialChatLlavaQwenConfig
                if overwrite_config is not None:
                    llava_cfg = AerialChatLlavaQwenConfig.from_pretrained(model_path)
                    rank0_print(f"Overwriting config with {overwrite_config}")
                    for k, v in overwrite_config.items():
                        setattr(llava_cfg, k, v)
                    model = AerialChatLlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                else:
                    model = AerialChatLlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
            elif "qwen" in model_name.lower() or "quyen" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from llava.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
