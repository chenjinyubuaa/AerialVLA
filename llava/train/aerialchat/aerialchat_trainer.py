...
from transformers.trainer import is_sagemaker_mp_enabled, get_parameter_names, has_length, ALL_LAYERNORM_LAYERS, logger, is_accelerate_available, is_datasets_available, GradientAccumulationPlugin, unwrap_model, _is_peft_model, MODEL_FOR_CAUSAL_LM_MAPPING_NAMES
...
class AerialChatTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.args.use_dagger:
            ...
            meta_info_list = inputs['metas']['meta_info_list']
            for i, meta_info in enumerate(meta_info_list):
                pred_action = dict()
                for k in pred_actions:
                    pred_action[k] = pred_actions[k][i].detach().cpu().float().numpy()
                meta_info['new_action'] = pred_action

            now_time = ('-').join(str(datetime.datetime.now()).split(' '))
            save_name = f"{now_time}.npy"
            torch.save(meta_info_list, os.path.join(self.args.save_augment_output_dir, save_name))
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            super(AerialChatTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            pass
        else:
            super(AerialChatTrainer, self)._save(output_dir, state_dict)
