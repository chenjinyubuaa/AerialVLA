...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, ACTION_TOKEN_INDEX
from llava.train.aerialchat.aerialchat_trainer import AerialChatTrainer
from llava.train.llava_trainer import LLaVATrainer

from llava import conversation as conversation_lib
from llava.model import *
from llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token, print_trainable_parameters
...
class ModelArguments:
    ...
    # For GeoChat Add
    stage1_path: str = field(default="./output")

    # For AerialChat Action Space
    use_grid_action: bool = field(default=False)
    grid_offset_size: int = field(default=9)
    grid_altitude_size: int = field(default=20)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # For GeoChat Add
    training_stage: int = field(default=1, metadata={"help": "GeoChat training stage."})
    # For AerialChat Dagger
    use_dagger: bool = field(default=False)
    dagger_epoch_num: int = field(default=1, metadata={"help": "AerialChat training stage."})
    dagger_data_rate: float = field(default=1.0, metadata={"help": "AerialChat training stage."})
    # For AerialChat Evaluate in Training
    evaluate_in_training: bool = field(default=False)
...
def find_all_linear_names(model):
    ...
    action_keywords = ['waypoint_decoder', 'altitude_decoder', 'progress_decoder', 'grid_decoder', 'grid_image_decoder', 'emb_embeddings']
    for name, module in model.named_modules():
        if any(mm_keyword in name for mm_keyword in multimodal_keywords):
            continue
        if any(action_keyword in name for action_keyword in action_keywords):
            continue
...
# change for aerialchat
origin_system = "You are a helpful assistant."
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message = aerialchat_system) -> Dict:
    if system_message is None:
        system_message = origin_system
    ...
    for i, source in enumerate(sources):
        ...
        for idx, encode_id in enumerate(input_id):
            ...
            # if encode_id == action_token_index:
            #     input_id[idx] = ACTION_TOKEN_INDEX
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, system_message=None) -> Dict:
    """
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file=None, overwrite_image_aspect_ratio=None, image=None):
    ...
    def process_aerial_image(self, image_file, meta, overwrite_image_aspect_ratio=None):
        image_folder = self.data_args.image_folder
        ...
        try:
            ...
            from llava.eval.aerialchat.utils import crop_image_from_corners, crop_corner_for_map
            history_map_image, corner_info = crop_image_from_corners(image, meta['current_corner_list'], meta['current_direction_list'], meta, resize_shape=None)
        ...
        image_list = []
        image_list.append(self.process_image(image=Image.fromarray(history_map_image)))
        image_list.append(self.process_image(image=Image.fromarray(current_image), overwrite_image_aspect_ratio="pad"))
        
        return image_list, corner_info
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        elif 'map_name' in sources[0]:
            image_file = self.list_data_dict[i]["map_name"] + ".tif"
            image, corner_info = self.process_aerial_image(image_file, self.list_data_dict[i]['meta'])
        ...
        has_image = ("image" in self.list_data_dict[i]) or ("video" in self.list_data_dict[i]) or ("map_name" in self.list_data_dict[i])
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image, system_message=system_message)
        ...
        if "image" in self.list_data_dict[i]:
            ...
            data_dict["image"] = image
        elif "map_name" in self.list_data_dict[i]:
            data_dict["image"] = image
        ...
        meta = self.list_data_dict[i]["meta"]
        
        if 'grid' in meta['target'] and meta['target']['grid'] is not None:
            grid_length = 9
            # 1. Generate the one-hot tensor
            _index = meta['target']['grid'][1] * grid_length + meta['target']['grid'][0]
            one_hot = torch.zeros(grid_length * grid_length)
            one_hot[_index] = 1
            one_hot = one_hot.view(grid_length, grid_length)

            kernel_size = 5
            # 2. Generate the Gaussian kernel
            ...
            # 3. Apply the Gaussian kernel to the one-hot tensor using convolution
            conv = nn.Conv2d(1, 1, kernel_size=kernel_size, padding='same', bias=False)
            with torch.no_grad():
                conv.weight[:] = kernel.view(1, 1, kernel_size, kernel_size)
                one_hot = one_hot.unsqueeze(0).unsqueeze(0).float()
                one_hot = conv(one_hot).squeeze(0).squeeze(0).flatten()

            meta['target']['grid_onehot'] = one_hot.cpu().detach().numpy()

            grid_altitude_length = 20
            one_hot = torch.zeros(grid_altitude_length)
            one_hot[meta['target']['grid_altitude']] = 1

            conv_1d = nn.Conv1d(1, 1, kernel_size=kernel_size, padding='same', bias=False)
            kernel_1d = torch.sum(kernel, dim=-1, keepdim=True)
            with torch.no_grad():
                conv_1d.weight[:] = kernel_1d.view(1, 1, kernel_size)
                one_hot = one_hot.unsqueeze(0).unsqueeze(0).float()
                one_hot = conv_1d(one_hot).squeeze(0).squeeze(0).flatten()

            meta['target']['grid_altitude_onehot'] = one_hot.cpu().detach().numpy()
        
        meta['map_name'] = self.list_data_dict[i]['map_name']
        meta['route_index'] = self.list_data_dict[i]['route_index']
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "meta" in instances[0]:
            keys = instances[0]['meta']['target'].keys()
            target = dict()
            for k in keys:
                if instances[0]['meta']['target'][k] is None:
                    continue
                value_list = []
                for instance in instances:
                    value_list.append(torch.tensor(instance['meta']['target'][k]))
                target[k] = torch.stack(value_list, dim=0)
                
            meta_info_list = [instance['meta']['meta_info'] for instance in instances]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    ### for lora training 
    overwrite_config["mm_tunable_parts"] = None
    ...
    # For AerialChat 
    overwrite_config["use_grid_action"] = model_args.use_grid_action
    overwrite_config["grid_offset_size"] = model_args.grid_offset_size
    overwrite_config["grid_altitude_size"] = model_args.grid_altitude_size
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if "moe" in model_args.model_name_or_path.lower() or "A14B" in model_args.model_name_or_path:
                ...
                model = AerialChatLlavaQwenForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        if training_args.training_stage == 2:
            model = load_lora(model, model_args.stage1_path)
    ...
    if not training_args.use_dagger:
        ...
        origin_data_path = data_args.data_path
        for epoch in range(training_args.dagger_epoch_num):
            if epoch != 0:
                data_args.data_path = os.path.join(training_args.output_dir, 'data_augment.json')
            training_args.output_dir = os.path.join(origin_output_dir, f'epoch_{epoch}')
            training_args.save_augment_output_dir = os.path.join(origin_output_dir, f'epoch_{epoch}', 'argument_datas')
            if training_args.local_rank == 0 or training_args.local_rank == -1:
                os.makedirs(training_args.save_augment_output_dir, exist_ok=True)
            
            data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args)
            trainer = AerialChatTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
            trainer.train()
            
            # save augment data
            if training_args.local_rank == 0 or training_args.local_rank == -1:
                from llava.eval.aerialchat.proc_instruction_aug import transforms_augment_data

                if model_args.use_grid_action:
                    grid_size = model_args.grid_offset_size
                    altitude_grid_size = model_args.grid_altitude_size
                else:
                    grid_size=None
                    altitude_grid_size=None

                transforms_augment_data(
                    origin_data_path=origin_data_path,
                    aug_data_path=training_args.save_augment_output_dir,
                    output_data_path=os.path.join(training_args.output_dir, 'data_augment.json'),
                    aug_data_rate=training_args.dagger_data_rate,
                    grid_size=grid_size,
            ...
            rank0_print(f"Model saved to {training_args.output_dir}")
            rank0_print(f"Evaluating model for dagger epoch {epoch}")
            # eval
            if training_args.evaluate_in_training:
                from llava.eval.aerialchat.inference import eval_model
                num_chunks = dist.get_world_size()
                chunk_idx = dist.get_rank()
                training_args.num_chunks = num_chunks
                training_args.chunk_idx = chunk_idx
                training_args.image_folder = data_args.image_folder

                eval_model(training_args, model, tokenizer, vision_tower.image_processor)
